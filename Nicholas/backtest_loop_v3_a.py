...
ANCHORS = [
    ...
    {"symbol": "BTC", "timeframe": "4H", "lag": 0},
]

# Base BUY rules (we’ll override change_pct in the loop)
BUY_RULES = [
    {"symbol": "BTC", "timeframe": "1H", "lag": 4, "change_pct": -4.5, "direction": "down"},
    {"symbol": "ETH", "timeframe": "1H", "lag": 4, "change_pct": -4.5, "direction": "down"},
    {"symbol": "SOL", "timeframe": "1H", "lag": 4, "change_pct": -4.5, "direction": "down"},
]

# SELL rules remain fixed
SELL_RULES = [
    {"symbol": "BTC", "timeframe": "1H", "lag": 0, "change_pct": -2.0, "direction": "down"},
]
...
for sym in symbols:
    ...
    for btc_cp in np.arange(-4.5, 15.5, 1):
        for eth_cp in np.arange(-2.0, 5.5, 1):  # Loop over ETH_CP from -2.0 to 5.0
            temp_buy = [{**r, 'change_pct': btc_cp} if r['symbol'] == "BTC" else
                        {**r, 'change_pct': eth_cp} if r['symbol'] == "ETH" else
                        r for r in BUY_RULES]
            ...
            results.append({
                "Symbol": sym,
                "BTC_cp": btc_cp,
                "ETH_cp": eth_cp,  # Add ETH_cp to the results
